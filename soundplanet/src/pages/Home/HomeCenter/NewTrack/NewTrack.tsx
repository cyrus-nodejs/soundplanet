...
import { fetchNewTrack, getNewTracks, } from "../../../../redux/features/audio/audioSlice";
import { fetchAddToPlaylist, fetchCreatePlaylist, getPlaylist, fetchPlaylist } from "../../../../redux/features/playlist/playlistSlice";
...
const newTracks = useAppSelector(getNewTracks)
...
<div className="ms-auto p-2"><Link to="/newtracks" className="text-decoration-none text-light">Show all</Link>  </div>
       ...
       <div className="row   ">
  {newTracks.slice(0, 4).map((track:TRACK) =>{
     return (
...
<ContextMenu.Item onSelect={HandleSelect} className="ContextMenuItem bg-dark"  id="something"onClick={() => dispatch(fetchCreatePlaylist())}>Create Playlist</ContextMenu.Item>
) }  
